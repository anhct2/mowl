...
def construct_society(question: str) -> RolePlaying:
    ...
    tools = [
        ...
        *SandboxToolkit(sandbox_manager=sandbox_manager).get_tools(),
    ]
    ...
    # Create script automation
    script_automation = ScriptAutomation(model=models["assistant"])
    ...
    # Enhance the society with script automation and sandbox integration
    enhanced_society = EnhancedOwlRolePlaying(
        original_role_playing=society,
        script_automation=script_automation
    )
    
    # Initialize with the task
    enhanced_society.init_with_task(question)
    
    return enhanced_society
...
def main():
    ...
    answer, chat_history, token_count = enhanced_run_society(society)
    ...
    # Output script progress if available
    if "script_progress" in token_count:
        print(f"\033[92mScript Progress: {token_count['script_progress']}\033[0m")
